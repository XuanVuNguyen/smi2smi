...
from smi2smi.transformer_ver_1 import build_model
...
training_config = {
    ...
    'save_dir':'model_checkpoints/Aug27_23h39_reproduce/',
    'initial_epoch':0}

token_lookup = TokenLookup(os.path.join(training_config['data_dir'], 'vocab'))

train_source_chrs = file_to_chrs(os.path.join(training_config['data_dir'], 'train_sources'))
train_target_chrs = file_to_chrs(os.path.join(training_config['data_dir'], 'train_targets'))
valid_source_chrs = file_to_chrs(os.path.join(training_config['data_dir'], 'valid_sources'))
valid_target_chrs = file_to_chrs(os.path.join(training_config['data_dir'], 'valid_targets'))
...
if training_config['initial_epoch']==0:
    ...
    with open(os.path.join(training_config['save_dir'], 'a-params.txt'), 'w') as params_file:
        json.dump({'model_config':model_config, 'training_config':training_config}, params_file, indent=4)
else:
    with open(os.path.join(training_config['save_dir'], 'a-params.txt'), 'r') as params_file:
