...
class MultiChannelConv1D(layers.Layer):
    def __init__(self,
        ...
        self.cnns = [layers.Conv1D(
            ...
            padding='same',
            activation='relu',
            name=f'cnn_size_{kernel_size}') for kernel_size in kernel_sizes]
        self.cnn_outputs_mix = layers.Dense(model_dim, name='cnn_outputs_mix')
    ...
    def call(self, inputs, mask=None):
        ...
        outputs = layers.Concatenate()(inputs_features)
        outputs = self.cnn_outputs_mix(outputs)
        outputs = self.dropout(outputs)
        outputs = self.layernorm(layers.Add()([outputs, inputs]))
        if mask is not None:
...
class PositionalEmbedding(layers.Layer):
    def __init__(self,
                 model_dim,
                 name='positional_embedding', **kwargs):
        super().__init__(name=name, **kwargs)
...
class Encoder(layers.Layer):
    def __init__(self, 
        ...
        self.embedding = layers.Embedding(vocab_size, block_config['model_dim'],
                                          mask_zero=True)
        self.inception_layers = [MultiChannelConv1D(
            kernel_sizes=conv_config['kernel_sizes'],
            n_filters=conv_config['n_filters'],
            model_dim=block_config['model_dim'],
            dropout=dropout,
            name=f'inception_{i}') for i in range(conv_config['depth'])]
        self.positional_embedding = PositionalEmbedding(block_config['model_dim'])
        self.input_dropout = layers.Dropout(rate=dropout)
    ...
    def call(self, inputs):
        ...
        embedded_inputs = self.positional_embedding(embedded_inputs)
...
class Decoder(layers.Layer):
    ...
    def call(self, dec_inputs, enc_outputs):
        ...
        embedded_dec_inputs = self.positional_embedding(embedded_dec_inputs)
        embedded_dec_inputs = self.input_dropout(embedded_dec_inputs)
...
def build_model(model_config):
    ...
    encoder = Encoder(encoder_depth=model_config['encoder_depth'],
                      ...
                      conv_config=model_config['conv_config'],
...
class InferenceDecoderBlock(DecoderBlock):
    def __init__(self,
                 ...
                 name='decoder_block',
                 **kwargs):
        super().__init__(block_config, 0.,name, **kwargs)
...
#                                         activation='softmax',
    ...
    def call(self, token, step, cache):
        ...
        cur_block_token = pos_embedded_token #shape==(batch, 1, feats)
...
class InferenceModel(keras.Model):
    def __init__(self,
        ...
        if model_config['embedding_share']:
            dec_embedding_layer = self.encoder.embedding_layer
        else:
            dec_embedding_layer = None
